...
Page({
  ...
  insertActivity: function() {
    ...
    that.data.user_OpenID = that.data.userInfo.user_OpenID
    that.data.activity_leader = that.data.userInfo.user_nickName
    that.data.user_avatarUrl = that.data.userInfo.user_avatarUrl
    that.data.user_verified = that.data.userInfo.user_verified

    console.log("imgpath")
    console.log(that.data.imggroup)
    console.log("type" + that.data.activity_type)
    ...
    console.log(that.data.activity_payment)
    console.log(that.data.activity_title)
    console.log(that.data.user_OpenID)
    console.log(that.data.activity_leader)
    console.log(that.data.user_avatarUrl)
    wx.request({
      url: 'https://www.monodark.cn/test2-maven/insertActivityInfoNew', //仅为示例，并非真实的接口地址        
      data: {
      ...
      success(res) {
        ...
        app.globalData.activityEdit = null;
        wx.redirectTo({
          url: '../myActivity/myActivity?removeSto=true',
        })
        wx.showToast({
          title: '活动发布成功',
          icon: 'success',
  ...
  topreviewActivity: function(event) {
    ...
    wx.navigateTo({
      url: '../previewActivity/previewActivity?' +
